...
def build_story(markdown):
    ...
    while i < len(lines):
        ...
        if line.startswith("| "):
            ...
            if len(block) <= 9 and story and isinstance(story[-1], Paragraph) and story[-1].style.name == "H2":
                story.extend([KeepTogether([story.pop(), Spacer(1, 1 * mm), table]), Spacer(1, 4 * mm)])
            else:
